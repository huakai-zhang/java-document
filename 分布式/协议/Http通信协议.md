...
2. 把 CSR 文件和其他可能的证件上传到 CA 认证机构，CA 机构收到证书申请之后，进行审核，主要检查网站地址的拥有者是否是证书的申请者 ，<font color=red>然后使用申请中的 Hash 算法，对部分内容进行摘要（相当于证书的唯一编号），然后使用 CA 机构自己的私钥对这段内容进行签名</font>

...
```java
// ScheduledFutureTask 包含 3 个成员变量
...
1. 线程 1 通过`自旋`从 DelayQueue 中获取已到期的 Task(到期任务指 time 大于等于当前时间)，整个过程使用 `ReenTrantLock` 加锁和 `Condition` 等待
   * 如果 PriorityQueue 为空，则线程等待
   * 如果头元素的 time 比当前时间大，等待到 time 时间
